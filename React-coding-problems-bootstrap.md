...
### Fetch inside useEffect with Cancellation
```tsx
useEffect(() => {
...
### Using React Query or SWR for Caching and Dedupe
```tsx
...
const { data, error, isLoading } = useSWR('/api/todos', u => fetch(u).then(r => r.json()));
```

Both handle caching, deduping, and revalidation automatically.
...
## SSR, SSG, and ISR (Next.js)

| Mode | When HTML Generated | Updates | Typical Use |
|------|----------------------|----------|--------------|
| SSR | On every request | Always fresh | Dynamic or user-specific pages |
| SSG | At build time | Manual rebuild | Blogs, docs |
| ISR | At build + revalidate interval | Automatic regeneration | Semi-static content |
